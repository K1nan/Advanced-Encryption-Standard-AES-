#include <iostream>
#include <iomanip>
#include <sstream>
#include <cstdlib>
#include <vector>

unsigned char SubstationBox[256] = {
    
};
unsigned char Multiplication222[256] = {
    
};
unsigned char Multiplication333[256] = {
    
};

unsigned char RoundKeys[176]; // 11 round keys * 16 bytes

void hexToBinary(const std::string& hex, unsigned char* binary) {
    for (size_t i = 0; i < hex.size(); i += 2) {
        std::string byteString = hex.substr(i, 2);
        binary[i / 2] = (unsigned char)strtol(byteString.c_str(), nullptr, 16);
    }
}

unsigned char rcon[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

void GenerateRoundKeys(unsigned char* key) {
    unsigned char temp[4];
    int i = 0;
    while (i < 16) {
        RoundKeys[i] = key[i];
        i++;
    }
    i = 16;
    int rconIndex = 0;

    while (i < 176) {
        for (int j = 0; j < 4; j++) {
            temp[j] = RoundKeys[i - 4 + j];
        }
        if (i % 16 == 0) {
            unsigned char t = temp[0];
            temp[0] = temp[1];
            temp[1] = temp[2];
            temp[2] = temp[3];
            temp[3] = t;

            for (int j = 0; j < 4; j++) 
            {
                temp[j] = SubstationBox[temp[j]];
            }
            temp[0] ^= rcon[rconIndex++];
        }
        for (int j = 0; j < 4; j++) {
            RoundKeys[i] = RoundKeys[i - 16] ^ temp[j];
            i++;
        }
    }
}


void SubstituteBytes(unsigned char* Block) {
    for (int i = 0; i < 16; i++) {
        Block[i] = SubstationBox[Block[i]];
    }
}


void ShiftRows(unsigned char* Block) {
    unsigned char tmp[16];

    tmp[0] = Block[0];
    tmp[1] = Block[5];
    tmp[2] = Block[10];
    tmp[3] = Block[15];

    tmp[4] = Block[4];
    tmp[5] = Block[9];
    tmp[6] = Block[14];
    tmp[7] = Block[3];

    tmp[8] = Block[8];
    tmp[9] = Block[13];
    tmp[10] = Block[2];
    tmp[11] = Block[7];

    tmp[12] = Block[12];
    tmp[13] = Block[1];
    tmp[14] = Block[6];
    tmp[15] = Block[11];

    for (int i = 0; i < 16; i++) Block[i] = tmp[i];
}

void MixColumns(unsigned char* Block) {
    unsigned char tmp[16];
    for (int i = 0; i < 4; i++) {
        int col = i * 4;
        tmp[col] = Multiplication222[Block[col]] ^ Multiplication333[Block[col + 1]] ^ Block[col + 2] ^ Block[col + 3];
        tmp[col + 1] = Block[col] ^ Multiplication222[Block[col + 1]] ^ Multiplication333[Block[col + 2]] ^ Block[col + 3];
        tmp[col + 2] = Block[col] ^ Block[col + 1] ^ Multiplication222[Block[col + 2]] ^ Multiplication333[Block[col + 3]];
        tmp[col + 3] = Multiplication333[Block[col]] ^ Block[col + 1] ^ Block[col + 2] ^ Multiplication222[Block[col + 3]];
    }
    for (int i = 0; i < 16; i++) {
        Block[i] = tmp[i];
    }
}

void AddRoundKey(unsigned char* Block, unsigned char* RoundKey) {
    for (int i = 0; i < 16; i++) {
        Block[i] ^= RoundKey[i];
    }
}
void AES_algorithm(unsigned char* message, unsigned char* key) {
    unsigned char Block[16];
    for (int i = 0; i < 16; i++) Block[i] = message[i];

    GenerateRoundKeys(key);
    AddRoundKey(Block, RoundKeys);

    for (int i = 1; i <= 9; i++) {
        SubstituteBytes(Block);
        ShiftRows(Block);
        MixColumns(Block);
        AddRoundKey(Block, RoundKeys + (16 * i));
    }
    SubstituteBytes(Block);
    ShiftRows(Block);
    AddRoundKey(Block, RoundKeys + 160);

    for (int i = 0; i < 16; i++) message[i] = Block[i];
}

int main() {
    std::vector<uint8_t> data;
    uint8_t key[16];
    char charecter;


    for (int i = 0; i < 16 && std::cin.get(charecter); i++) {
        key[i] = static_cast<uint8_t>(charecter);
    }

    while (std::cin.get(charecter)) {
        data.push_back(static_cast<uint8_t>(charecter));
    }

    for (size_t i = 0; i < data.size(); i += 16) {
        uint8_t message[16] = { 0 };
        std::copy_n(data.begin() + i, std::min<size_t>(16, data.size() - i), message);
        AES_algorithm(message, key);
        std::cout.write(reinterpret_cast<char*>(message), 16);
    }

    return 0;
}

/*int main() {
    std::string MessageHex = "F295B9318B994434D93D98A4E449AFD8";
    std::string KeyHex = "F4C020A0A1F604FD343FAC6A7E6AE0F9";
    
    std::string MessageHex, KeyHex;
    std::cin >> MessageHex >> KeyHex;
    unsigned char Message[16];
    unsigned char Key[16];
    hexToBinary(MessageHex, Message);
    hexToBinary(KeyHex, Key);

    AES_algorithm(Message, Key);
    for (int i = 0; i < 16; i++)
    {
        std::cout  << std::hex << std::setw(2) << std::setfill('0')   << (int)Message[i];
    }

    return 0;
}*/